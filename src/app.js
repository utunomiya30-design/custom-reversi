...
function renderResult({ syncOnline = true, scroll = true } = {}) { const values = state.winMode === "score" ? scoreBoard(state.board, state.playerCount, state.scoreMap) : countStones(state.board, state.playerCount); const ranking = rankPlayers(values, state.winMode === "reverse" ? "reverse" : "classic"); const unit = state.winMode === "score" ? dict().points : dict().stones; els.ranks.replaceChildren(); ranking.forEach((entry, i) => { const li = document.createElement("li"); li.innerHTML = `<span>#${i + 1} ${label(entry.player)}</span><span class="rank-value">${entry.value} ${unit}</span>`; els.ranks.append(li); }); els.play.classList.add("is-hidden"); els.result.classList.remove("is-hidden"); if (scroll) scrollToPanel(els.result); if (syncOnline) syncRoom(); }
