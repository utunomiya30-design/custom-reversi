...
function applyRoom(room) { if (!room || suppressRoomSync) return; assignedPlayer = roomClient.getAssignedPlayer(); updateOfflineCpuPlayers(room); applySettings(room.rules); if (!room.board) return updateOnlineStatus(); const wasPlaying = !els.play.classList.contains("is-hidden"); state = { ...room.rules, board: room.board, currentPlayer: room.currentPlayer, scoreMap: createScoreMap(room.rules.boardSize), finished: room.finished, lastMessage: room.lastMessage ?? "" }; els.setup.classList.add("is-hidden"); els.play.classList.toggle("is-hidden", room.finished); els.result.classList.toggle("is-hidden", !room.finished); room.finished ? finish() : render(); if (!room.finished && !wasPlaying) scrollToPanel(); queueCpu(); }
