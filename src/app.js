...
const COLORS = { 1: "#141414", 2: "#f7f3ea", 3: "#d65353", 4: "#3f6fd8" };
const LANG = {
  ja: {
...
function stone(x, y, radius, color, light) { const g = ctx.createRadialGradient(x - radius * .3, y - radius * .35, radius * .1, x, y, radius); g.addColorStop(0, light ? "#fff" : "#555"); g.addColorStop(1, color); ctx.fillStyle = g; ctx.beginPath(); ctx.arc(x, y, radius, 0, Math.PI * 2); ctx.fill(); ctx.strokeStyle = "rgba(0,0,0,.28)"; ctx.stroke(); }
