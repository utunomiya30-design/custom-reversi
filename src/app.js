...
function stone(x, y, radius, color, light) { if (color === COLORS[3] || color === COLORS[4]) { ctx.fillStyle = color; ctx.beginPath(); ctx.arc(x, y, radius, 0, Math.PI * 2); ctx.fill(); ctx.strokeStyle = "#111"; ctx.lineWidth = 2.5; ctx.stroke(); return; } const g = ctx.createRadialGradient(x - radius * .3, y - radius * .35, radius * .1, x, y, radius); g.addColorStop(0, light ? "#fff" : "#555"); g.addColorStop(1, color); ctx.fillStyle = g; ctx.beginPath(); ctx.arc(x, y, radius, 0, Math.PI * 2); ctx.fill(); ctx.strokeStyle = "rgba(0,0,0,.35)"; ctx.stroke(); }
