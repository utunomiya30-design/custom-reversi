...
import { LocalRoomClient, ensureOnlineAuth, isOnlineSignedIn } from "./online-local-room.js?v=20260529-guest-online-1";
...
const LANG = {
  ja: {
    ...
    ad: "Advertisement", largeAd: "Interstitial / Large Banner", seat: "担当", wait: "参加待ち", spectator: "観戦中", blocked: "オンライン対戦では自分のターンだけ操作できます", notFound: "部屋が見つかりません", loginRequired: "ゲストとしてオンライン接続中...",
  },
  en: {
    ...
    ad: "Advertisement", largeAd: "Interstitial / Large Banner", seat: "Seat", wait: "Waiting", spectator: "Spectating", blocked: "Online games only allow moves on your own turn", notFound: "Room not found", loginRequired: "Connecting as guest...",
  },
};
...
const els = {
  ...
  canvas: $("#gameCanvas"), resultCanvas: $("#resultBoardCanvas"), turn: $("#turnLabel"), log: $("#moveLog"), scores: $("#scoreRow"), invite: $("#inviteButton"), restart: $("#restartButton"), back: $("#backButton"), ranks: $("#rankingList"), resultShare: $("#shareResultButton"),
};
...
function startGame({ syncOnline = true } = {}) { clearCpu(); state = createState(settings()); els.setup.classList.add("is-hidden"); els.result.classList.add("is-hidden"); els.play.classList.remove("is-hidden"); render(); scrollToPanel(); if (syncOnline) syncRoom(); queueCpu(); }
...
function markerIndexes(size) { const inner = Math.max(1, Math.floor(size / 4)); const outer = size - inner; return inner === outer ? [inner] : [inner, outer]; }
function drawBoardSurface(targetCtx, width, height) { const base = targetCtx.createLinearGradient(0, 0, width, height); base.addColorStop(0, "#2d765b"); base.addColorStop(.42, "#18553f"); base.addColorStop(1, "#0c3429"); targetCtx.fillStyle = base; targetCtx.fillRect(0, 0, width, height); const glow = targetCtx.createRadialGradient(width * .18, height * .16, 0, width * .18, height * .16, width * .82); glow.addColorStop(0, "rgba(255,255,255,.18)"); glow.addColorStop(.46, "rgba(255,255,255,.035)"); glow.addColorStop(1, "rgba(0,0,0,.22)"); targetCtx.fillStyle = glow; targetCtx.fillRect(0, 0, width, height); targetCtx.save(); targetCtx.globalAlpha = .16; for (let y = 0; y < height; y += 18) { targetCtx.strokeStyle = y % 36 === 0 ? "rgba(255,255,255,.22)" : "rgba(0,0,0,.18)"; targetCtx.beginPath(); targetCtx.moveTo(0, y + Math.sin(y * .08) * 2); targetCtx.bezierCurveTo(width * .28, y + 7, width * .68, y - 7, width, y + Math.cos(y * .05) * 3); targetCtx.stroke(); } targetCtx.restore(); }
function drawBoardGrid(targetCtx, size, width, height) { const c = width / size; targetCtx.lineWidth = Math.max(1, Math.floor(c * .018)); targetCtx.strokeStyle = "rgba(235, 226, 199, .58)"; for (let i = 0; i <= size; i++) { const x = Math.round(i * c) + .5; targetCtx.beginPath(); targetCtx.moveTo(x, 0); targetCtx.lineTo(x, height); targetCtx.stroke(); targetCtx.beginPath(); targetCtx.moveTo(0, x); targetCtx.lineTo(width, x); targetCtx.stroke(); } const dots = markerIndexes(size); targetCtx.fillStyle = "rgba(246, 236, 205, .82)"; targetCtx.strokeStyle = "rgba(21, 55, 42, .42)"; for (const row of dots) for (const col of dots) { const x = col * c; const y = row * c; const r = Math.max(3.2, c * .045); targetCtx.beginPath(); targetCtx.arc(x, y, r, 0, Math.PI * 2); targetCtx.fill(); targetCtx.stroke(); } }
function drawBoardImage(canvas, board, size, currentPlayer = null, showHints = false) { const targetCtx = canvas?.getContext("2d"); if (!targetCtx || !board) return; const width = canvas.width, height = canvas.height, c = width / size; targetCtx.clearRect(0, 0, width, height); drawBoardSurface(targetCtx, width, height); if (bgImage) { targetCtx.globalAlpha = .24; targetCtx.drawImage(bgImage, 0, 0, width, height); targetCtx.globalAlpha = 1; } drawBoardGrid(targetCtx, size, width, height); const legal = showHints && currentPlayer ? new Set(getLegalMoves(board, currentPlayer).map(m => `${m.row},${m.col}`)) : new Set(); for (let r = 0; r < size; r++) for (let col = 0; col < size; col++) { const p = board[r][col], x = col * c + c / 2, y = r * c + c / 2; if (p) stone(targetCtx, x, y, c * .38, COLORS[p], p === 2, pieceImages.get(p)); else if (legal.has(`${r},${col}`)) hint(targetCtx, x, y, c * .13); } }
function draw() { drawBoardImage(els.canvas, state.board, state.boardSize, state.currentPlayer, true); }
function stone(targetCtx, x, y, radius, color, light, image = null) { const lighting = color === COLORS[3] ? { highlight: "#ff9a9a", shadow: "#b40000", edge: "rgba(180,0,0,.28)" } : color === COLORS[4] ? { highlight: "#9fc3ff", shadow: "#003aa8", edge: "rgba(0,58,168,.28)" } : null; targetCtx.save(); targetCtx.shadowColor = "rgba(0,0,0,.34)"; targetCtx.shadowBlur = radius * .18; targetCtx.shadowOffsetY = radius * .08; const g = targetCtx.createRadialGradient(x - radius * .3, y - radius * .35, radius * .1, x, y, radius); g.addColorStop(0, lighting?.highlight ?? (light ? "#fff" : "#555")); g.addColorStop(.58, color); g.addColorStop(1, lighting?.shadow ?? color); targetCtx.fillStyle = g; targetCtx.beginPath(); targetCtx.arc(x, y, radius, 0, Math.PI * 2); targetCtx.fill(); targetCtx.restore(); if (image?.complete) { targetCtx.save(); targetCtx.beginPath(); targetCtx.arc(x, y, radius * .9, 0, Math.PI * 2); targetCtx.clip(); const side = radius * 1.8; targetCtx.drawImage(image, x - side / 2, y - side / 2, side, side); targetCtx.restore(); const shine = targetCtx.createRadialGradient(x - radius * .38, y - radius * .42, 1, x, y, radius); shine.addColorStop(0, "rgba(255,255,255,.52)"); shine.addColorStop(.38, "rgba(255,255,255,.12)"); shine.addColorStop(1, "rgba(0,0,0,.18)"); targetCtx.fillStyle = shine; targetCtx.beginPath(); targetCtx.arc(x, y, radius, 0, Math.PI * 2); targetCtx.fill(); } targetCtx.strokeStyle = lighting?.edge ?? (light ? "rgba(0,0,0,.32)" : "rgba(0,0,0,.5)"); targetCtx.lineWidth = image ? 1.4 : (lighting ? 1.2 : 2.5); targetCtx.stroke(); }
function hint(targetCtx, x, y, r) { targetCtx.fillStyle = "rgba(255,246,216,.68)"; targetCtx.beginPath(); targetCtx.arc(x, y, r, 0, Math.PI * 2); targetCtx.fill(); }
...
function renderResult({ syncOnline = true, scroll = true } = {}) { const values = state.winMode === "score" ? scoreBoard(state.board, state.playerCount, state.scoreMap) : countStones(state.board, state.playerCount); const ranking = rankPlayers(values, state.winMode === "reverse" ? "reverse" : "classic"); const unit = state.winMode === "score" ? dict().points : dict().stones; drawBoardImage(els.resultCanvas, state.board, state.boardSize, null, false); els.ranks.replaceChildren(); ranking.forEach((entry, i) => { const li = document.createElement("li"); li.innerHTML = `<span>#${i + 1} ${label(entry.player)}</span><span class="rank-value">${entry.value} ${unit}</span>`; els.ranks.append(li); }); els.play.classList.add("is-hidden"); els.result.classList.remove("is-hidden"); if (scroll) scrollToPanel(els.result); if (syncOnline) syncRoom(); }
function finish({ syncOnline = true } = {}) { clearCpu(); state.finished = true; renderResult({ syncOnline }); }
function setRoomClient(client, key) { if (unsub) unsub(); if (roomClient) roomClient.close(); document.body.dataset.onlineRoomPath = "manualRoomsV1"; roomClient = client; unsub = roomClient.subscribe(applyRoom); assignedPlayer = roomClient.getAssignedPlayer(); els.roomCode.value = roomClient.roomId.toUpperCase(); els.online.textContent = `${key}: ${roomClient.roomId.toUpperCase()}`; updateRoomUrl(); }
function applyRoom(room) { if (!room || suppressRoomSync) return; assignedPlayer = roomClient.getAssignedPlayer(); updateOfflineCpuPlayers(room); applySettings(room.rules); if (!room.board) return updateOnlineStatus(); const wasPlaying = !els.play.classList.contains("is-hidden"); state = { ...room.rules, board: room.board, currentPlayer: room.currentPlayer, scoreMap: createScoreMap(room.rules.boardSize), finished: room.finished, lastMessage: room.lastMessage ?? "" }; els.setup.classList.add("is-hidden"); if (room.finished) { clearCpu(); renderResult({ syncOnline: false, scroll: wasPlaying }); updateOnlineStatus(); return; } els.play.classList.remove("is-hidden"); els.result.classList.add("is-hidden"); render(); if (!wasPlaying) scrollToPanel(); queueCpu(); }
...
async function requireOnlineLogin() { if (isOnlineSignedIn()) return true; els.online.textContent = dict().loginRequired; await ensureOnlineAuth(); return true; }
async function createRoom() { try { if (!await requireOnlineLogin()) return; setRoomClient(LocalRoomClient.createRoom({ rules: settings() }), dict().createRoom); startGame(); } catch (error) { els.online.textContent = `ゲスト接続に失敗しました。Firebase Authenticationの匿名ログインを有効にしてください。(${error?.code || "unknown"})`; } }
async function joinRoom(id) { try { if (!await requireOnlineLogin()) return; const forceNewClient = new URLSearchParams(location.search).get("join") === "1"; setRoomClient(LocalRoomClient.joinRoom({ roomId: id.toLowerCase(), forceNewClient }), dict().join); } catch (error) { els.online.textContent = error?.message === "ONLINE_AUTH_REQUIRED" ? "ゲスト接続に失敗しました。" : dict().notFound; } }
...
els.share.addEventListener("click", shareUrl); els.invite.addEventListener("click", shareUrl);
...
els.restart.addEventListener("click", startGame); els.back.addEventListener("click", () => { clearCpu(); delete document.body.dataset.onlineRoomPath; els.play.classList.add("is-hidden"); els.result.classList.add("is-hidden"); els.setup.classList.remove("is-hidden"); scrollToPanel(els.setup); });
